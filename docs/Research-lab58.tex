...
\textbf{Data:} BET index daily closing prices (2010-2024)

\textbf{Size:} 3,533 observations after cleaning

\textbf{Features created:} 12 technical indicators
\begin{itemize}
    \item 5 lagged volatilities ($\sigma_{t-1}^2, \ldots, \sigma_{t-20}^2$)
    \item 4 moving averages (MA 5, 10, 20, 50)
    \item RSI, momentum, volatility of volatility
\end{itemize}

\textbf{Target:} Binary high/low volatility (75th percentile threshold)

\textbf{Note:} Google Trends features not included yet (data collection in progress)
...
\hline
\end{tabular}
\caption{XGBoost baseline performance without Google Trends}
\end{table}
...
\begin{enumerate}
    \item Volatility of volatility: 0.180
    \item Momentum: 0.099
    \item RSI: 0.081
    \item MA(20): 0.076
    \item MA(10): 0.074
    \item Lagged volatility (20 days): 0.074
    \item MA(5): 0.074
    \item MA(50): 0.073
    \item Lagged volatility (10 days): 0.069
    \item Lagged volatility (5 days): 0.068
\end{enumerate}
...
\begin{itemize}
    \item Model learns patterns successfully (training AUC 0.98)
    \item Test AUC 0.68 is reasonable baseline without Google Trends
    \item Volatility of volatility emerges as strongest predictor
    \item Good precision for low volatility detection (0.80)
\end{itemize}

\subsubsection{Limitations}

\begin{itemize}
    \item Clear overfitting (training 0.98 vs test 0.68)
    \item Poor recall for high volatility spikes (only 12\% detected)
    \item Class imbalance challenges (75\% low vol, 25\% high vol)
    \item No behavioral features (Google Trends) included yet
\end{itemize}
...
\begin{enumerate}
    \item Add Google Trends features (expected +0.05-0.10 AUC)
    \item Apply preprocessing pipeline (clustering, smoothing, detrending)
    \item Hyperparameter tuning via grid search
    \item Address class imbalance (adjust class weights or use SMOTE)
